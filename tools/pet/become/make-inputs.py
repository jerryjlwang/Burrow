...
# A phone photo of a pencil drawing: unevenly lit paper, a dark desk edge, faint grey lines with a gap in the outline.
photo = Image.new("RGB", (900, 900), (218, 212, 200))
arr = np.array(photo).astype(np.int16)
arr = arr + np.linspace(-45, 10, 900)[None, :, None] + np.linspace(-10, 15, 900)[:, None, None] + np.random.randint(-3, 4, (900, 900, 1))
photo = Image.fromarray(np.clip(arr, 0, 255).astype(np.uint8))
d = ImageDraw.Draw(photo)
d.rectangle((0, 0, 55, 900), fill=(70, 58, 50))
d.rectangle((0, 840, 900, 900), fill=(150, 142, 130))
P = (112, 106, 100)
cx, cy = 480, 470
d.ellipse((cx - 190, cy - 130, cx + 190, cy + 190), outline=P, width=4)
d.polygon([(cx - 175, cy - 60), (cx - 250, cy - 330), (cx - 60, cy - 120)], outline=P, width=4)
d.polygon([(cx + 175, cy - 60), (cx + 250, cy - 330), (cx + 60, cy - 120)], outline=P, width=4)
d.polygon([(cx - 250, cy - 330), (cx - 215, cy - 230), (cx - 160, cy - 275)], fill=(60, 55, 52))
d.polygon([(cx + 250, cy - 330), (cx + 215, cy - 230), (cx + 160, cy - 275)], fill=(60, 55, 52))
d.ellipse((cx - 110, cy - 40, cx - 60, cy + 10), fill=(60, 55, 52))
d.ellipse((cx + 60, cy - 40, cx + 110, cy + 10), fill=(60, 55, 52))
d.ellipse((cx - 12, cy + 40, cx + 12, cy + 58), fill=(70, 65, 60))
d.ellipse((cx - 160, cy + 50, cx - 100, cy + 110), outline=P, width=3)
d.ellipse((cx + 100, cy + 50, cx + 160, cy + 110), outline=P, width=3)
d.arc((cx - 40, cy + 60, cx + 40, cy + 120), 10, 170, fill=P, width=3)
# A gap in the head outline, like a pencil skipping.
d.rectangle((cx - 12, cy + 183, cx + 12, cy + 196), fill=(216, 210, 198))
photo = photo.filter(ImageFilter.GaussianBlur(0.9))
photo.save(here / "pencil-drawing.jpg", quality=82)
print("wrote pencil-drawing.jpg")
