...
export function Board() {
  ...
  const dragFrame = (
    <>
      <div className="pip-board-box" data-hot={hot ? "true" : "false"} style={{ position: "absolute", inset: -BOX_PAD, border: "1.5px dashed rgba(255,255,255,.95)", boxShadow: "0 0 0 1px rgba(0,0,0,.6), inset 0 0 0 1px rgba(0,0,0,.6)", borderRadius: 8, opacity: hot ? 1 : 0, transition: "opacity 120ms", pointerEvents: "none" }} />
      <div className="pip-board-edge" {...grab} style={{ ...handle, left: out, right: out, top: out, height: EDGE }} />
      <div className="pip-board-edge" {...grab} style={{ ...handle, left: out, right: out, bottom: out, height: EDGE }} />
      <div className="pip-board-edge" {...grab} style={{ ...handle, top: out, bottom: out, left: out, width: EDGE }} />
      <div className="pip-board-edge" {...grab} style={{ ...handle, top: out, bottom: out, right: out, width: EDGE }} />
      <div className="pip-board-grip" role="button" aria-label="Drag the drawing" title="Drag to move" {...grab} style={{ ...handle, top: -10, right: 18, width: 22, height: 22, borderRadius: 11, background: "rgba(0,0,0,.55)", color: "#fff", fontSize: 13, lineHeight: "22px", textAlign: "center", userSelect: "none" }}>
        ✥
      </div>
    </>
  );
  ...
  if (rect && region) {
    ...
    return (
      <div ref={wrapper} className="pip-board" role="figure" aria-label="drawing" data-placement="video" style={{ ...frame, left: rect.x, top: rect.y, width: rect.width, height: rect.height, display: "flex", flexDirection: "column", gap: 6, padding: 8, boxSizing: "border-box", borderRadius: 10, background: region.busy > EMPTY_BUSY ? "rgba(0,0,0,.5)" : undefined }}>
        {textBlock}
  ...
  if (rect) {
    ...
    return (
      <div ref={wrapper} className="pip-board" role="figure" aria-label="drawing" data-placement="element" style={{ ...frame, left: rect.x, top: rect.y, width: rect.width, height: rect.height }}>
  ...
  return (
    <div ref={wrapper} className="pip-board" role="figure" aria-label="drawing" data-placement="corner" style={{ ...frame, left: 20, bottom: 20, width: 380 }}>
      {textBlock}
