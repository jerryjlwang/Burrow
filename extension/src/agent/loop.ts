  ...
  async run(utterance: string, opts: RunOptions): Promise<void> {
    ...
    try {
      ...
      for (; step < maxStep; step++) {
        ...
        const input = this.buildInput({ utterance, goal, conversation: session.recentTurns(10), page, history, pendingOffer, path, learner, tablet, step, maxStep, withFrame: firstStep && taskType !== "navigation" && taskType !== "administrative", resumed: step === resumedStep });
