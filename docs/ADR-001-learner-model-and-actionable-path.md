...
| Problem plan (the question on screen) | Also `profile.plans` (`kind: "problem"`, own cap of 40 so homework can't evict asked-for plans), but only once the student **engages** — writes working, answers, or takes a hint. Pages merely visited are never stored. Keyed by the problem, so the same problem met on two pages is one record. | The step judge's `planStep`, or a graded correct answer (`planProgress` event, by key — evidence on a concept never completes someone else's problem). |

**Provenance.** Every stored plan carries `provenance` (origin `asked` with the learner's own words, or `page` with the URL and title; which planner made it), every done step carries a `completion` (`resource` with the URL and title opened, `attempt`, or `working`, plus when), and the plan keeps a capped `history` of `created` / `step_done` / `wrong_step` / `solved`. Re-saving a plan never rewrites its origin, completions or history. This is the record a future study-plan console reads; nothing about it is tied to a tab or session. On return, `recallPlan` restores the plan and progress from memory synchronously, with no planner call.
