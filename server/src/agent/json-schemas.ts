...
export const DECISION_JSON_SCHEMA = {
  ...
  properties: {
    ...
    elementId: nullable({ type: "integer" }, "Target element id from INTERACTIVE ELEMENTS. Required for highlight, point_to, focus, clear, select, press_enter, scroll_to. click, double_click, right_click, hover and drag take it OR x,y. type and press_key take it to focus that element first, or null to use whatever is focused. observe takes it to read that region in full; sketch takes it to wrap the drawing onto that element. Otherwise null."),
    text: nullable(str(), "Text to type (type); the key or chord (press_key), e.g. 'Escape', 'ArrowDown', 'Control+a'; the question (ask_user); a longer written explanation for the panel (explain); 'screenshot' to request visual context (observe); the search query (look_up); the topic the student wants to learn (make_plan); for sketch the lines to draw — or, with value \"erase\", \"all\" / the item numbers to remove; or for video the time to seek to ('6:40', or '+10' / '-10' seconds from here) or the speed ('0.75'). Otherwise null."),
    ...
    value: nullable(str(), "Option label or value for select; for sketch, \"add\" extends the drawing already on screen instead of replacing it and \"erase\" removes from it (text = \"all\" or item numbers); for video, the control: \"play\", \"pause\", \"seek\" or \"speed\". Otherwise null."),
