...
export const DECISION_JSON_SCHEMA = {
  ...
  properties: {
    ...
    text: nullable(str(), "Text to type (type); the key or chord (press_key), e.g. 'Escape', 'ArrowDown', 'Control+a'; the question (ask_user); a longer written explanation for the panel (explain); 'screenshot' to request visual context (observe); the search query (look_up); the topic the student wants to learn (make_plan); or for sketch the lines to draw — or, with value \"erase\", \"all\" / the item numbers to remove. Otherwise null."),
