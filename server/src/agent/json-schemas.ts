...
export const DECISION_JSON_SCHEMA = {
  ...
  properties: {
    ...
    x: nullable({ type: "number" }, "Pointer target: CSS pixels from the viewport's left edge, which are exactly the screenshot's pixels. With y, an alternative to elementId for click, double_click, right_click, hover and drag (the drag start); for scroll, the point to wheel over (scrolls that pane, zooms that map). Otherwise null."),
  ...
  required: ["action", "say", "elementId", "text", "url", "direction", "amount", "value", "quote", "line", "tabId", "x", "y", "toElementId", "toX", "toY", "pendingAction", "taskType", "reason", "done"],
  additionalProperties: false,
} as const;
